...
void Render();
HRESULT UpdateWaveMesh();

// Hated Global variables
...
HRESULT UpdateWaveMesh()
{
	...
	ZeroMemory(&bd, sizeof(bd));
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(SimpleVertex) * WaveGen->GetNumVertices();
	...
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(int) * WaveGen->GetNumIndices();        // 36 vertices needed for 12 triangles in a triangle list
	bd.BindFlags = D3D11_BIND_INDEX_BUFFER;
	bd.CPUAccessFlags = 0;
	InitData.pSysMem = WaveGen->GetVertIndices();
